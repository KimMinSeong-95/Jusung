...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        bSave = CreateWindowW(_T("BUTTON"), _T("저장"),
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
            300, 20, 100, 25,
            hWnd, (HMENU)ID_BTN_SAVE, hInst, NULL);

        bPrint = CreateWindowW(_T("BUTTON"), _T("인쇄"),
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
            300, 50, 100, 25,
    ...
    case WM_KEYDOWN:
    {
        ...
        case VK_LEFT:
            ...
            if (bShow)
            {
                bShow = FALSE;
                ShowWindow(hEdit, 0);
                SetWindowText(hEdit, _T("사라졌다"));
                SetWindowText(bSave, _T("사라졌다"));
            }
            else
            {
                bShow = TRUE;
                ShowWindow(hEdit, 1);
                SetWindowText(hEdit, _T("나타났다"));
                SetWindowText(bSave, _T("나타났다"));
            }
       /* case VK_LEFT:
            ShowWindow(hEdit,0);
            break;
    ...
    break;
    case WM_LBUTTONDOWN:
        SetParent(bSave, GetDesktopWindow());
        SetWindowText(bSave, _T("가출한 버튼"));
        break;
    case WM_RBUTTONDOWN:
        SetParent(bSave, hWnd);
        SetWindowText(bSave, _T("돌아온 버튼"));
        break;
    case WM_PAINT:
    {
        PAINTSTRUCT ps;
